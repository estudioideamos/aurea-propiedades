...
export function SiteFooter() {
  ...
  const [footerExploreOpen, setFooterExploreOpen] = useState(false);
  ...
  useEffect(() => { void fetch("/api/settings", { cache:"no-store" }).then(response=>response.ok?response.json():null).then(data=>{ if(data?.settings) setSettings(data.settings); }).catch(()=>{}); }, []);
  ...
  return <><footer className="site-footer rich-footer">
    ...
    <div className="rich-footer-main"><div className="rich-footer-brand"><Brand/><p>Una inmobiliaria boutique que combina arquitectura, estrategia y cercan&iacute;a para acompa&ntilde;ar decisiones que importan.</p><div className="footer-social"><a href="https://instagram.com" target="_blank" rel="noreferrer" aria-label="Instagram"><Instagram aria-hidden="true" strokeWidth={1.7}/></a><a href="https://linkedin.com" target="_blank" rel="noreferrer" aria-label="LinkedIn"><Linkedin aria-hidden="true" strokeWidth={1.7}/></a></div></div>
      <div className={"rich-footer-nav footer-nav-toggle "+(footerExploreOpen?"open":"")}><button type="button" onClick={()=>setFooterExploreOpen(value=>!value)} aria-expanded={footerExploreOpen}><span>EXPLORÁ</span><ChevronDown aria-hidden="true"/></button><div><Link href="/propiedades">Propiedades</Link><Link href="/emprendimientos">Emprendimientos</Link><Link href="/propiedades?operacion=venta">Comprar</Link><Link href="/propiedades?operacion=alquiler">Alquilar</Link><Link href="/tasacion">Vender</Link><Link href="/nosotros">Nuestra mirada</Link></div></div>
      <div className={"rich-footer-nav footer-nav-toggle "+(footerServicesOpen?"open":"")}><button type="button" onClick={()=>setFooterServicesOpen(value=>!value)} aria-expanded={footerServicesOpen}><span>SERVICIOS</span><ChevronDown aria-hidden="true"/></button><div><Link href="/servicios">Compra y venta</Link><Link href="/servicios">Inversiones</Link><Link href="/tasacion">Tasaciones</Link><Link href="/contacto">Contacto</Link><Link href="/admin">Administración</Link></div></div>
      <div className="rich-footer-contact"><p className="contact-kicker">ASESORAMIENTO PERSONAL</p><h3>&iquest;Te ayudamos a elegir?</h3><a className="footer-whatsapp" href={`https://wa.me/${phoneDigits}`} target="_blank" rel="noreferrer"><span>Hablar por WhatsApp</span><ArrowUpRight aria-hidden="true" strokeWidth={1.8}/></a><p><MapPin aria-hidden="true"/> {settings.address}</p><p><Clock3 aria-hidden="true"/> {settings.schedule}</p><a className="footer-email" href={`mailto:${settings.contactEmail}`}><Mail aria-hidden="true"/><span>{settings.contactEmail}</span></a></div>
    </div>
