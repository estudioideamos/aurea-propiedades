...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const { development } = await getLiveDevelopment(slug);
  if (!development) return { title: "Emprendimiento no encontrado | Ideamos Propiedades" };
  const title = `${development.title} | Ideamos Propiedades`;
  const description = `${development.status} en ${development.location}. ${formatDevelopmentPrice(development)}. Unidades ${development.units}.`;
  const url = `${siteUrl}/emprendimientos/${development.slug}`;
  const image = absoluteImage(development.image);
  return {
    title, description,
    alternates: { canonical: url },
    openGraph: { title, description, url, siteName: "Ideamos Propiedades", locale: "es_AR", type: "website", images: [{ url: image, alt: development.title }] },
...
export default async function DevelopmentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="detail-page premium-detail development-detail"><JsonLd data={developmentJsonLd(development)}/><SiteHeader/>
    ...
    <section className="detail-layout"><div className="detail-content">
      ...
      <section className="detail-block"><div className="detail-block-head"><div><p className="detail-block-label">02 / INFORMACI&Oacute;N GENERAL</p><h2>Datos del desarrollo</h2></div><Building2 aria-hidden="true" strokeWidth={1.5}/></div><div className="detail-data-grid"><div><span>PROYECTO</span><b>{development.title}</b></div><div><span>TIPOLOG&Iacute;AS</span><b>{development.units}</b></div><div><span>ESTADO DE OBRA</span><b>{development.status}</b></div><div><span>ENTREGA ESTIMADA</span><b>{development.delivery}</b></div><div><span>CANTIDAD DE PISOS</span><b>{development.floors}</b></div><div><span>UNIDADES</span><b>{development.apartments}</b></div><div><span>COCHERAS</span><b>{development.garages}</b></div><div><span>DESARROLLADOR</span><b>{development.developer}</b></div><div><span>ARQUITECTURA</span><b>{development.architect}</b></div></div></section>
      ...
      <section className="detail-block location-block"><div className="detail-block-head"><div><p className="detail-block-label">06 / UBICACI&Oacute;N</p><h2>{development.location}</h2></div><MapPin aria-hidden="true" strokeWidth={1.5}/></div><iframe title={`Mapa de ${development.location}`} src={mapUrl} loading="lazy" referrerPolicy="no-referrer-when-downgrade"/></section>
    </div><aside className="detail-sidebar"><div className="contact-card premium-contact"><p className="detail-block-label">RECIB&Iacute; INFORMACI&Oacute;N</p><h3>&iquest;Quer&eacute;s conocer el proyecto?</h3><p>Dejanos tus datos y te enviamos disponibilidad, planos, valores y forma de pago.</p><label><span>NOMBRE</span><input placeholder="Tu nombre"/></label><label><span>TEL&Eacute;FONO</span><input placeholder="+54 9 11"/></label><label><span>EMAIL</span><input placeholder="nombre@email.com" type="email"/></label><label><span>MENSAJE</span><textarea defaultValue={`Hola, me interesa el emprendimiento ${development.title}.`}/></label><button type="button">Solicitar informaci&oacute;n <ArrowUpRight aria-hidden="true" strokeWidth={1.8}/></button><a href={`https://wa.me/541155550190?text=${encodeURIComponent(`Hola, me interesa el emprendimiento ${development.title}.`)}`} target="_blank" rel="noreferrer"><img src={siteAsset("/assets/whatsapp.svg")} alt="" aria-hidden="true"/> Consultar por WhatsApp</a></div><div className="detail-advisor"><span>ASESORAMIENTO EN EMPRENDIMIENTOS</span><div className="advisor-avatar">ID</div><h4>Equipo Ideamos</h4><p>Inversi&oacute;n y nuevos desarrollos</p><a href="tel:+541155550190">+54 11 5555 0190</a></div></aside></section>
