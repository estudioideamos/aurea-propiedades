...
export async function getLiveProperties(): Promise<LiveProperty[]> {
  ...
  try {
    const { scheduleTokkoRefreshIfDue } = await import("./tokko-integration");
    scheduleTokkoRefreshIfDue();
  } catch { /* La cartera guardada sigue disponible aunque la actualizacion automatica no pueda iniciarse. */ }
  try {
    ...
    return rows.map(row => ({
      id: row.id, slug: row.slug, title: row.title, location: row.location, zone: row.zone as Property["zone"], operation: row.operation as Property["operation"], type: row.type as Property["type"], currency: row.currency as Property["currency"], price: row.price, pricePrefix: row.pricePrefix, priceSuffix: row.priceSuffix, expenses: row.expenses, expensesCurrency: row.expensesCurrency as Property["expensesCurrency"], rooms: row.rooms, bedrooms: row.bedrooms, bathrooms: row.bathrooms, area: row.area, image: row.image, description: row.description, amenities: parseStringArray(row.amenities), gallery: parseStringArray(row.gallery), coveredArea: row.coveredArea, garages: row.garages, age: row.age, condition: row.condition, orientation: row.orientation, featured: row.featured, status: row.status, updatedAt: row.updatedAt,
    }));
