...
export async function getLiveProperties(): Promise<LiveProperty[]> {
  ...
  try {
    ...
    return rows.map(row => ({
      id: row.id, slug: row.slug, title: row.title, location: row.location, zone: row.zone as Property["zone"], operation: row.operation as Property["operation"], type: row.type as Property["type"], currency: row.currency as Property["currency"], price: row.price, rooms: row.rooms, bedrooms: row.bedrooms, bathrooms: row.bathrooms, area: row.area, image: row.image, description: row.description, amenities: parseAmenities(row.amenities), featured: row.featured, status: row.status, updatedAt: row.updatedAt,
    }));
