...
export function PropertyAdmin({ initialProperties, userName, signOutPath }: Props) {
  ...
  return <main className="admin-app">
    <aside className={`admin-sidebar ${sidebarOpen ? "open" : ""}`}>
      ...
      <nav className="admin-navigation secondary"><p>CONFIGURACIÓN</p><button className={view === "settings" ? "active" : ""} type="button" onClick={() => navigate("settings")}><Settings/><span>Ajustes del sitio</span></button><a className="admin-nav-link" href={PUBLIC_SITE} target="_blank" rel="noreferrer"><Eye/><span>Ver sitio p?blico</span><ArrowUpRight/></a></nav>
      <div className="admin-user"><span>{userName.slice(0,2).toUpperCase()}</span><div><b>{userName}</b><small>Administración</small></div><a className="admin-user-logout" href={signOutPath}><LogOut/><span>Cerrar sesi?n</span></a></div>
    </aside>
