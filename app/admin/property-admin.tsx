...
const leadKindLabel: Record<AdminLead["kind"], string> = { property: "Propiedad", contact: "Contacto", valuation: "Tasación" };
...
const baseZones = ["CABA", "Zona Norte", "Zona Oeste", "Zona Sur", "Costa Atl\u00e1ntica", "Interior"];
...
export function PropertyAdmin({ initialProperties, userName, signOutPath }: Props) {
  ...
  const typeSuggestions = useMemo(() => Array.from(new Set(["Casa", "Departamento", "PH", "Terreno", ...items.map(item => item.type).filter(Boolean)])).sort((a, b) => a.localeCompare(b, "es")), [items]);
  const zoneSuggestions = useMemo(() => Array.from(new Set([...baseZones, ...items.map(item => item.zone).filter(Boolean)])).sort((a, b) => a.localeCompare(b, "es")), [items]);
  ...
  return <main className="admin-app">
    ...
    {editorOpen && <div className="admin-editor-layer" role="dialog" aria-modal="true" aria-label={draft.id?"Editar propiedad":"Nueva propiedad"}><button className="admin-editor-backdrop" type="button" onClick={()=>setEditorOpen(false)} aria-label="Cerrar editor"/><form className="admin-editor" onSubmit={submit}><header><div><p>{draft.id?`REF. ID-${String(draft.id).padStart(4,"0")}`:"NUEVA PUBLICACIÓN"}</p><h2>{draft.id?"Editar propiedad":"Crear propiedad"}</h2></div><button type="button" onClick={()=>setEditorOpen(false)} aria-label="Cerrar"><X/></button></header><div className="admin-editor-progress"><button type="button" className={editorStep===1?"active":"complete"} onClick={()=>changeEditorStep(1)}><b>1</b>{"Informaci\u00f3n"}</button><i/><button type="button" className={editorStep===2?"active":editorStep>2?"complete":""} onClick={()=>changeEditorStep(2)} disabled={!canContinueEditor}><b>2</b>{"Ficha t\u00e9cnica"}</button><i/><button type="button" className={editorStep===3?"active":""} onClick={()=>changeEditorStep(3)} disabled={!canContinueEditor}><b>3</b>{"Im\u00e1genes"}</button></div><div className="admin-editor-body">
      {editorStep===1&&<section><div className="admin-form-title"><span>01</span><div><h3>Información principal</h3><p>Identidad, ubicación y condiciones comerciales.</p></div></div><div className="admin-form-grid"><label className="wide"><span>NOMBRE DE LA PROPIEDAD</span><input value={draft.title} onChange={event=>updateDraft("title",event.target.value)} required placeholder="Ej. Casa del Lago"/></label><label><span>OPERACIÓN</span><select value={draft.operation} onChange={event=>updateDraft("operation",event.target.value as Property["operation"])}><option>Venta</option><option>Alquiler</option><option>Alquiler temporario</option></select></label><label><span>TIPO</span><input list="property-type-options" value={draft.type} onChange={event=>updateDraft("type",event.target.value)} placeholder="Elegir o escribir un tipo"/><datalist id="property-type-options">{typeSuggestions.map(option=><option value={option} key={option}/>)}</datalist></label><label><span>UBICACIÓN</span><input value={draft.location} onChange={event=>updateDraft("location",event.target.value)} required placeholder="Barrio, ciudad"/></label><label><span>ZONA</span><input list="property-zone-options" value={draft.zone} onChange={event=>updateDraft("zone",event.target.value)} placeholder="Elegir o escribir una zona"/><datalist id="property-zone-options">{zoneSuggestions.map(option=><option value={option} key={option}/>)}</datalist></label><label><span>MONEDA</span><select value={draft.currency} onChange={event=>updateDraft("currency",event.target.value as Property["currency"])}><option>USD</option><option>ARS</option></select></label><div className="admin-price-composer wide"><label><span>TEXTO ANTES</span><input value={draft.pricePrefix??""} onChange={event=>updateDraft("pricePrefix",event.target.value)} placeholder="Ej. Desde"/></label><label><span>PRECIO</span><input type="number" min="0" value={draft.price} onChange={event=>updateDraft("price",Number(event.target.value))}/></label><label><span>TEXTO DESPUÉS</span><input value={draft.priceSuffix??""} onChange={event=>updateDraft("priceSuffix",event.target.value)} placeholder="Ej. x mes"/></label></div><div className="admin-expenses-composer wide"><div><b>EXPENSAS MENSUALES</b><small>Opcional. Dejalo en cero si la propiedad no tiene expensas.</small></div><label><span>MONEDA</span><select value={draft.expensesCurrency??"ARS"} onChange={event=>updateDraft("expensesCurrency",event.target.value as Property["expensesCurrency"])}><option>ARS</option><option>USD</option></select></label><label><span>IMPORTE</span><input type="number" min="0" value={draft.expenses??0} onChange={event=>updateDraft("expenses",Number(event.target.value))} placeholder="0"/></label></div><label className="wide"><span>ENLACE / SLUG</span><input value={draft.slug} onChange={event=>updateDraft("slug",slugify(event.target.value))} placeholder="Se genera automáticamente"/></label></div></section>}
