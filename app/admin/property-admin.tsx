...
export function PropertyAdmin({ initialProperties, userName, signOutPath }: Props) {
  ...
  const openCreate = () => { setDraft(toDraft()); setEditorOpen(true); };
  const openEdit = (property: AdminProperty) => { setDraft(toDraft(property)); setEditorOpen(true); };
  const openDuplicate = (property: AdminProperty) => { setDraft({ ...toDraft(property), id: undefined, title: `${property.title} copia`, slug: "", status: "draft" }); setEditorOpen(true); };
  ...
  return <main className="admin-app">
    ...
    </div><footer><button type="button" onClick={()=>setEditorOpen(false)}>Cancelar</button><div><button type="button" onClick={()=>void persistProperty("draft")} disabled={saving}><FileText/> Guardar borrador</button><button className="primary" type="submit" disabled={saving||uploading}>{saving?"Guardando...":<><Check/> Guardar propiedad</>}</button></div></footer></form></div>}
  </main>;
}
