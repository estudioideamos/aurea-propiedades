...
export function HomeExperience({ properties }: { properties: Property[] }) {
  ...
  return <main className="vian-site"><SiteHeader/>
    ...
    <section className="testimonial-section testimonial-carousel"><div className="testimonial-orbit" aria-hidden="true"><span className="testimonial-orbit-text">{testimonialOrbitText.split("").map((char,index)=><i key={char+"-"+index} style={{transform:"rotate("+(15+(index*(330/Math.max(testimonialOrbitText.length-1,1))))+"deg)"}}>{char===" "?"\u00a0":char}</i>)}</span><span className="testimonial-orbit-core"><img src={siteAsset("/assets/aurea-hero-vian.png")} alt=""/><b>&ldquo;</b></span></div><p className="testimonial-kicker">CLIENTES / EXPERIENCIAS / CONFIANZA</p><div className="testimonial-slide" key={testimonialIndex}><blockquote>{testimonials[testimonialIndex].quote}</blockquote><div className="quote-author"><b>{testimonials[testimonialIndex].author}</b><span>{testimonials[testimonialIndex].role}</span></div></div><div className="testimonial-controls"><button type="button" onClick={previousTestimonial} aria-label="Testimonio anterior"><ChevronLeft aria-hidden="true"/></button><div>{testimonials.map((_,index)=><button type="button" key={index} className={index===testimonialIndex?"active":""} onClick={()=>setTestimonialIndex(index)} aria-label={"Ver testimonio "+(index+1)}/>)}</div><button type="button" onClick={nextTestimonial} aria-label="Testimonio siguiente"><ChevronRight aria-hidden="true"/></button></div></section>
