...
const testimonials = [
  ...
  { quote: "Desde la primera visita hasta la firma todo fue ordenado, humano y transparente. Ideamos hizo que una decisión enorme se sintiera liviana.", author: "SOFÍA Y TOMÁS", role: "COMPRADORES / NORDELTA" }
];
...
export function HomeExperience({ properties }: { properties: Property[] }) {
  ...
  return <main className="vian-site"><SiteHeader/>
    ...
    <section className="properties-section" id="propiedades"><div className="vian-section-head"><div><span className="section-tag">03 / PROPIEDADES DESTACADAS</span><h2>Lugares para vivir.<br/>Activos para crecer.</h2></div><div className="property-head-actions"><div className="filter-row">{(["Venta","Alquiler","Alquiler temporario"] as const).map(item=><button key={item} className={operation===item?"active":""} onClick={()=>setOperation(item)}>{item}</button>)}</div><Link href="/propiedades" className="outline-button">Ver las {properties.length} <ArrowUpRight className="mini-arrow" aria-hidden="true"/></Link></div></div><div className="property-grid">{visible.slice(0,6).map((p,i)=><PropertyCard key={p.id} property={p} index={i}/>)}</div></section>
    <section className="metrics-section"><div className="metrics-image"><img src="https://images.unsplash.com/photo-1600047509807-ba8f99d2cdde?auto=format&fit=crop&w=1600&q=88" alt="Arquitectura residencial contemporanea"/><div><p>NUESTRO COMPROMISO</p><h2>Resultados que construyen confianza.</h2></div></div><div className="metrics-grid"><article><MetricCounter value={480} suffix="+"/><p>Operaciones acompa&ntilde;adas</p></article><article><MetricCounter value={96} suffix="%"/><p>Clientes que nos recomiendan</p></article><article><MetricCounter value={24} suffix="h"/><p>Tiempo medio de respuesta</p></article><article><MetricCounter value={properties.length}/><p>Propiedades activas hoy</p></article></div></section>
