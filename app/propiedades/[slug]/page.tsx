...
export default async function PropertyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="detail-page premium-detail"><SiteHeader/>
    ...
    <section className="detail-layout"><div className="detail-content">
      ...
      <section className="detail-block"><div className="detail-block-head"><div><p className="detail-block-label">02 / RESUMEN GENERAL</p><h2>Datos principales</h2></div><Ruler aria-hidden="true" strokeWidth={1.5}/></div><div className="detail-data-grid"><div><span>ID DE PROPIEDAD</span><b>AUR-{String(property.id).padStart(4,"0")}</b></div><div><span>OPERACION</span><b>{property.operation}</b></div><div><span>TIPO</span><b>{property.type}</b></div><div><span>AMBIENTES</span><b>{property.rooms}</b></div><div><span>DORMITORIOS</span><b>{property.bedrooms}</b></div><div><span>BA&Ntilde;OS</span><b>{property.bathrooms}</b></div><div><span>SUPERFICIE CUBIERTA</span><b>{covered?`${covered} m2`:"A definir"}</b></div><div><span>SUPERFICIE TOTAL</span><b>{property.area} m2</b></div><div><span>COCHERAS</span><b>{garage}</b></div><div><span>ANTIGUEDAD</span><b>{age}</b></div><div><span>ESTADO</span><b>{property.condition || "Excelente"}</b></div><div><span>ORIENTACION</span><b>{property.orientation || "Norte"}</b></div></div></section>
