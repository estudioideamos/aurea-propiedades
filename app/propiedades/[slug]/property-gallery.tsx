...
export function PropertyGallery({ images, title }: PropertyGalleryProps) {
  ...
  return <>
    <section className="detail-gallery-grid" aria-label={"Galería de " + title}>
      {images.map((image, index) => <button type="button" onClick={() => setActive(index)} key={image + "-" + index} className={index === 0 ? "gallery-main" : "gallery-small gallery-" + index} aria-label={"Abrir foto " + (index + 1) + " de " + images.length}>
        <img src={image} alt={index === 0 ? title : title + ", vista " + (index + 1)}/>
        {index === 0 && <span>Imagen principal</span>}
        {index === images.length - 1 && <span className="gallery-count">VER GALERÍA / {String(images.length).padStart(2, "0")}</span>}
      </button>)}
    </section>
